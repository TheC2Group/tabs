...
(function (global, factory) {
	...
	(global = global || self, global.Tabs = factory());
	...
	var addAriaAttributes = function addAriaAttributes() {
	  ...
	  this.$tabs.each((i, tab) => {
	    ...
	    if (!tabId) {
	      ...
	      jquery(tab).attr('data-original-id', true);
	    }
	  });
	  this.$panels.each((i, panel) => {
	    var labelledBy = jquery(panel).attr('aria-labelledby');
	    jquery(panel).attr({
	      'role': 'tabpanel',
	      'tabindex': i === this.index ? 0 : -1,
	    ...
	    if (!labelledBy) {
	      ...
	      jquery(panel).attr('data-original-labelledBy', true);
	    }
	  });
	};
	...
	var removeAriaAttributes = function removeAriaAttributes() {
	  ...
	  this.$tabs.each((i, tab) => {
	    ...
	    if (!jquery(tab).attr('data-original-id')) {
	      jquery(tab).removeAttr('id');
	    }

	    jquery(tab).removeAttr('role tabindex aria-selected data-original-id');
	  });
	  this.$panels.each((i, panel) => {
	    var labelledBy = jquery(panel).attr('aria-labelledby');

	    if (!jquery(panel).attr('data-original-labelledBy')) {
	      jquery(panel).removeAttr('aria-labelledby');
	    }

	    jquery(panel).removeAttr('role tabindex aria-hidden data-original-labelledBy');
	  });
	};
